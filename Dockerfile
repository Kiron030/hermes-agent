...
# Harden startup for Windows/local deploy paths:
...
# Powerunits internal deployment: fail-closed first-boot runtime policy.
# See docker/apply_powerunits_runtime_policy.py + docker/stage2-hook.sh.
...
# No VOLUME: Railway rejects VOLUME in Dockerfile; mount /opt/data via
# Railway Volumes instead (see commit 16fac6cde).
