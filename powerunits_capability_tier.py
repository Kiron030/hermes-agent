...
Reads ``HERMES_POWERUNITS_CAPABILITY_TIER`` from the environment (integer ``0``–``6``).
...
- **5:** Adds **Tier 4B** — ``powerunits_tier4b_review_governance`` (review-state + workspace governance scaffolding; detail doc).
- **6:** Adds **Tier 5A** — ``powerunits_tier5a_bounded_workflow_scaffolding`` (bounded operator workflow run records + checkpoints; no bounded HTTP from tools; detail doc).
...
def read_powerunits_capability_tier() -> int:
    """Return an integer in ``0``..``6`` from ``HERMES_POWERUNITS_CAPABILITY_TIER``.
...
def log_startup_capability_tier_notice() -> None:
    ...
    if tier >= 6:
        extra = (
            "; Tier 4A–5A overlays eligible (tier>=4 drafts; tier>=5 governance; tier>=6 workflow scaffolding)"
        )
    elif tier >= 5:
        extra = (
            "; Tier 4A+4B overlays eligible (tier>=4 drafts; tier>=5 governance/review-state)"
        )
    elif tier >= 4:
