...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    - Direct api.openai.com endpoints need the Responses API for GPT-5.x
      tool calls with reasoning (chat/completions returns 400). Whether
      ``reasoning.encrypted_content`` is safe to request is decided
      downstream by ``agent._model_requires_responses_api()`` (model-based,
      not URL-based) — see ``openai_encrypted_reasoning_ok`` in
      ``agent/chat_completion_helpers.py``, which is what actually protects
      GPT-4.x models on OpenAI direct from the 400 on `/v1/responses`.
      Mapping the hostname to ``codex_responses`` here does not bypass
      that guard.
    ...
    if path.endswith("/anthropic") or path.endswith("/anthropic/v1"):
        return "anthropic_messages"
    # Kimi Code endpoint uses Anthropic Messages transport semantics.
...
def _resolve_openrouter_runtime(
    *,
    ...
    if _is_openrouter_context:
        ...
        # the canonical env var for ollama.com authentication. A base_url
        # that merely contains "ollama.com" in its path/query or whose
