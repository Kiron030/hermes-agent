...
over contiguous ≤7d windows (Repo B **Tier‑1** bounded ISO2 slice), fail-fast.
...
narrows besides Repo B enforcement (**env var omitted ⇒ implicit DE-only Hermes permits**).
"""
...
from tools.powerunits_era5_weather_bounded_execute_tool import (
    check_powerunits_era5_weather_bounded_execute_requirements,
...
from tools.powerunits_era5_weather_bounded_summary_tool import (
...
_FEATURE_ENV = "HERMES_POWERUNITS_ERA5_WEATHER_BOUNDED_CAMPAIGN_ENABLED"
_SURFACE = "powerunits_era5_weather_bounded_campaign"
...
def check_powerunits_era5_weather_bounded_campaign_requirements() -> bool:
...
def campaign_powerunits_era5_weather_bounded_de(
    *,
    ...
    base_statement = (
        ...
        "Powerunits `…/era5-weather/recompute` and `…/era5-weather/summary-window` "
        "per sub-window. Does not call market_feature_job, market_driver_feature_job, or "
        "expand_market_data."
    )

    if not check_powerunits_era5_weather_bounded_campaign_requirements():
        return json.dumps(
            {
                ...
                "message": (
                    ...
                    f"`{ERA5_WEATHER_BOUNDED_PRIMARY_ENV}` (or both legacy execute+summary flags "
                    f"`{ERA5_WEATHER_BOUNDED_LEGACY_ENV['execute']}` + "
                    f"`{ERA5_WEATHER_BOUNDED_LEGACY_ENV['summary']}`) with optional "
                    f"`{ERA5_WEATHER_BOUNDED_ALLOWED_COUNTRIES_ENV}` when primary is used; "
    ...
    try:
        cc, ver, windows = validate_era5_bounded_campaign(country_s, start_s, end_s, version_s)
    ...
    for idx, (w_start, w_end) in enumerate(windows):
        ...
        ex_raw = execute_powerunits_era5_weather_bounded_slice(
            country=cc,
            start=ws,
            end=we,
            version=ver,
        ...
        try:
            ...
            logger.warning("era5 bounded campaign: execute returned non-JSON")
            rows.append(
                {
                    "index": idx,
                    "slice": slice_echo,
        ...
        sum_raw = summarize_powerunits_era5_weather_bounded_window(
            country=cc,
            start=ws,
            end=we,
            version=ver,
...
CAMPAIGN_ERA5_SCHEMA = {
    ...
    "description": (
        ...
        "**Tier‑1 Repo B** ISO2 (intersects optional Hermes allowlist under primary gate). Fail-fast "
        "on first failed execute or failed summary HTTP/outcome. Requires "
...
registry.register(
    name="campaign_powerunits_era5_weather_bounded_de",
    toolset="powerunits_era5_weather_bounded_campaign",
    schema=CAMPAIGN_ERA5_SCHEMA,
    handler=lambda args, **kw: campaign_powerunits_era5_weather_bounded_de(
    ...
    check_fn=check_powerunits_era5_weather_bounded_campaign_requirements,
    requires_env=[
        _FEATURE_ENV,
        ERA5_WEATHER_BOUNDED_PRIMARY_ENV,
        ERA5_WEATHER_BOUNDED_ALLOWED_COUNTRIES_ENV,
        ERA5_WEATHER_BOUNDED_LEGACY_ENV["execute"],
        ERA5_WEATHER_BOUNDED_LEGACY_ENV["summary"],
