#!/usr/bin/env python3
"""
Hermes **DE** bounded summary-window for `market_features_hourly`.

POST `…/market-features-hourly/summary-window` with **`country_code: "DE"`**.
Gated by **`HERMES_POWERUNITS_MARKET_FEATURES_BOUNDED_ENABLED`** or legacy
**`HERMES_POWERUNITS_MARKET_FEATURES_BOUNDED_DE_SUMMARY_ENABLED`** — **not** PL Option D.
"""
...
_SURFACE = "powerunits_market_features_bounded_de_summary"
...
def check_powerunits_market_features_bounded_de_summary_requirements() -> bool:
...
def summarize_powerunits_market_features_bounded_de_window(
    *,
    ...
    base_statement = (
        "**DE** bounded summary-window — not PL Option D. Read-only Repo B POST; **`country_code=DE`**; "
        "same **≤24h** window as PL bounded family."
    )

    if not check_powerunits_market_features_bounded_de_summary_requirements():
        return json.dumps(
            {
    ...
    try:
        ...
        logger.warning("market_features DE bounded summary: HTTP timeout")
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
        ...
        logger.warning("market_features DE bounded summary: HTTP error %s", e)
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
...
SUMMARY_MF_DE_SCHEMA = {
    "name": "summarize_powerunits_market_features_bounded_de_window",
    "description": (
        "**Bounded DE `market_features_hourly` summary-window** — **DE** / **v1** / **≤24h**, then "
        ...
        "Optional `pipeline_run_id` from DE execute."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
registry.register(
    name="summarize_powerunits_market_features_bounded_de_window",
    toolset="powerunits_market_features_bounded_de_summary",
    schema=SUMMARY_MF_DE_SCHEMA,
    handler=lambda args, **kw: summarize_powerunits_market_features_bounded_de_window(
    ...
    check_fn=check_powerunits_market_features_bounded_de_summary_requirements,
    requires_env=[
        MARKET_FEATURES_BOUNDED_PRIMARY_ENV,
        MARKET_FEATURES_BOUNDED_ALLOWED_COUNTRIES_ENV,
        _LEGACY_ENV,
