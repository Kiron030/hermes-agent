#!/usr/bin/env python3
"""
Hermes bounded **post-execute validation** for Option D — one HTTP POST to Repo B.

Calls ``POST /internal/hermes/bounded/v1/market-features-hourly/validate-window``.
Read-only on the Powerunits side; no SQL from Hermes. Gated by
``HERMES_POWERUNITS_OPTION_D_VALIDATE_ENABLED`` plus base URL and bearer secret.
"""
...
_FEATURE_ENV = "HERMES_POWERUNITS_OPTION_D_VALIDATE_ENABLED"
...
_SURFACE = "powerunits_option_d_validate"
...
def check_powerunits_option_d_validate_requirements() -> bool:
...
def validate_powerunits_option_d_bounded_window(
    *,
    country: str,
    start: str,
    end: str,
    ...
    if not check_powerunits_option_d_validate_requirements():
        return json.dumps(
            {
    ...
    base_statement = (
        ...
        "Powerunits bounded internal validate-window API (read-only checks on "
        "`market_features_hourly` for the validated slice)."
    )
    ...
    try:
        ...
        logger.warning("option_d validate: HTTP timeout")
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
        ...
        logger.warning("option_d validate: HTTP error %s", e)
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
...
VALIDATE_OPTION_D_SCHEMA = {
    "name": "validate_powerunits_option_d_bounded_window",
    "description": (
        "**Option D bounded validate-window** — validates PL / v1 / ≤24h UTC locally, then one "
        "HTTP POST to Powerunits internal read-only validation. Requires "
        f"{_FEATURE_ENV}, {_BASE_ENV}, {_SECRET_ENV}. Optional `pipeline_run_id` from execute response."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "pipeline_run_id": {
                "type": "string",
                "description": "Optional UUID from execute response for pipeline_run echo.",
            },
        },
...
registry.register(
    name="validate_powerunits_option_d_bounded_window",
    toolset="powerunits_option_d_validate",
    schema=VALIDATE_OPTION_D_SCHEMA,
    handler=lambda args, **kw: validate_powerunits_option_d_bounded_window(
    ...
    check_fn=check_powerunits_option_d_validate_requirements,
    requires_env=[
        _FEATURE_ENV,
        _BASE_ENV,
        _SECRET_ENV,
