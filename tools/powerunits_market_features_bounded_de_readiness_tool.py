#!/usr/bin/env python3
"""
Hermes **DE** bounded readiness-window for `market_features_hourly` inputs.

POST `…/market-features-hourly/readiness-window` with **`country_code: "DE"`**.
Gated by **`HERMES_POWERUNITS_MARKET_FEATURES_BOUNDED_ENABLED`** or legacy
**`HERMES_POWERUNITS_MARKET_FEATURES_BOUNDED_DE_READINESS_ENABLED`** — **not** PL Option D.
"""
...
_SURFACE = "powerunits_market_features_bounded_de_readiness"
...
def readiness_powerunits_market_features_bounded_de_window(
    *,
    ...
    base_statement = (
        "**DE** bounded readiness — not PL Option D. Read-only Repo B POST; **`country_code=DE`**; "
        "same **≤24h** window as PL bounded `market-features-hourly` routes."
    )

    if not check_powerunits_market_features_bounded_de_readiness_requirements():
        return json.dumps(
            {
    ...
    try:
        ...
        logger.warning("market_features DE bounded readiness: HTTP timeout")
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
        ...
        logger.warning("market_features DE bounded readiness: HTTP error %s", e)
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
...
READINESS_MF_DE_SCHEMA = {
    "name": "readiness_powerunits_market_features_bounded_de_window",
    "description": (
        "**Bounded DE `market_features_hourly` readiness-window** — **DE** / **v1** / **≤24h**, then "
        ...
        "Optional `pipeline_run_id` may be ignored server-side."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
registry.register(
    name="readiness_powerunits_market_features_bounded_de_window",
    toolset="powerunits_market_features_bounded_de_readiness",
    schema=READINESS_MF_DE_SCHEMA,
    handler=lambda args, **kw: readiness_powerunits_market_features_bounded_de_window(
    ...
    check_fn=check_powerunits_market_features_bounded_de_readiness_requirements,
    requires_env=[
        MARKET_FEATURES_BOUNDED_PRIMARY_ENV,
        MARKET_FEATURES_BOUNDED_ALLOWED_COUNTRIES_ENV,
        _LEGACY_ENV,
