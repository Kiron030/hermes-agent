#!/usr/bin/env python3
"""
Hermes bounded **post-run summary** for Option D — one HTTP POST to Repo B.

Calls ``POST /internal/hermes/bounded/v1/market-features-hourly/summary-window``.
Read-only on the Powerunits side; no SQL from Hermes. Gated by
``HERMES_POWERUNITS_OPTION_D_SUMMARY_ENABLED`` plus base URL and bearer secret.
"""
...
_FEATURE_ENV = "HERMES_POWERUNITS_OPTION_D_SUMMARY_ENABLED"
...
_SURFACE = "powerunits_option_d_summary"
_SUMMARY_PATH = "/internal/hermes/bounded/v1/market-features-hourly/summary-window"
...
def check_powerunits_option_d_summary_requirements() -> bool:
...
def summarize_powerunits_option_d_bounded_window(
    *,
    country: str,
    start: str,
    end: str,
    ...
    if not check_powerunits_option_d_summary_requirements():
        return json.dumps(
            {
    ...
    base_statement = (
        ...
        "Powerunits bounded internal summary-window API (read-only readiness + validation "
    ...
    try:
        ...
        logger.warning("option_d summary: HTTP timeout")
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
        ...
        logger.warning("option_d summary: HTTP error %s", e)
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
...
SUMMARY_OPTION_D_SCHEMA = {
    "name": "summarize_powerunits_option_d_bounded_window",
    "description": (
        "**Option D bounded summary-window** — validates PL / v1 / ≤24h UTC locally, then one "
        "HTTP POST to Powerunits internal read-only summary (readiness + validation + optional "
        "pipeline run echo). Requires "
        f"{_FEATURE_ENV}, {_BASE_ENV}, {_SECRET_ENV}. Optional `pipeline_run_id` from execute."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "pipeline_run_id": {
                "type": "string",
                "description": "Optional UUID from execute response for execution block / echo.",
            },
        },
...
registry.register(
    name="summarize_powerunits_option_d_bounded_window",
    toolset="powerunits_option_d_summary",
    schema=SUMMARY_OPTION_D_SCHEMA,
    handler=lambda args, **kw: summarize_powerunits_option_d_bounded_window(
    ...
    check_fn=check_powerunits_option_d_summary_requirements,
    requires_env=[
        _FEATURE_ENV,
        _BASE_ENV,
        _SECRET_ENV,
