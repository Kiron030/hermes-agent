#!/usr/bin/env python3
"""
Hermes bounded ERA5 **coverage-scan** — read-only POST to Repo B (multi-subwindow).

Uses the same slice caps as the ERA5 bounded campaign (≤31d, ≤5 × ≤7d partitions),
validated locally before HTTP. Gated by ``HERMES_POWERUNITS_ERA5_WEATHER_BOUNDED_COVERAGE_SCAN_ENABLED``.

Does not run ``era5_weather_job``. Structured next-step suggestions appear only when present
in Repo B’s ``rollup.suggested_next_bounded_action``; Hermes does not infer them locally.
"""
...
_FEATURE_ENV = "HERMES_POWERUNITS_ERA5_WEATHER_BOUNDED_COVERAGE_SCAN_ENABLED"
...
_SURFACE = "powerunits_era5_weather_bounded_coverage_scan"
_SCAN_PATH = "/internal/hermes/bounded/v1/era5-weather/coverage-scan"
...
def check_powerunits_era5_weather_bounded_coverage_scan_requirements() -> bool:
...
def scan_powerunits_era5_weather_bounded_coverage_de(
    *,
    scan_start_utc: str,
    scan_end_utc: str,
    country: str = "DE",
    ...
    base_statement = (
        ...
        "Powerunits bounded `…/era5-weather/coverage-scan`. "
        "Did not run era5_weather_job (no ERA5 execute). "
        "market_feature_job and market_driver_feature_job were not run. "
        "Any rollup.suggested_next_bounded_action in the response comes from Repo B only; "
        "Hermes does not add local remediation suggestions."
    )

    if not check_powerunits_era5_weather_bounded_coverage_scan_requirements():
        return json.dumps(
            {
    ...
    try:
        cc, ver, _planned = validate_era5_bounded_campaign(
    ...
    try:
        ...
        logger.warning("era5 bounded coverage-scan: HTTP timeout")
        return json.dumps(
            {
                "surface": _SURFACE,
                "scan_attempted": True,
        ...
        logger.warning("era5 bounded coverage-scan: HTTP error %s", e)
        return json.dumps(
            {
                "surface": _SURFACE,
                "scan_attempted": True,
...
registry.register(
    name="scan_powerunits_era5_weather_bounded_coverage_de",
    toolset="powerunits_era5_weather_bounded_coverage_scan",
    schema=SCAN_ERA5_SCHEMA,
    handler=lambda args, **kw: scan_powerunits_era5_weather_bounded_coverage_de(
    ...
    check_fn=check_powerunits_era5_weather_bounded_coverage_scan_requirements,
