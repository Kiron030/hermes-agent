#!/usr/bin/env python3
"""
Hermes **DE** bounded validate-window for `market_driver_features_hourly`.

POST `…/market-driver-features-hourly/validate-window` with **`country_code: "DE"`**.
...
— **not** market-features **`HERMES_POWERUNITS_MARKET_FEATURES_BOUNDED_DE_*`**.
"""
...
_SURFACE = "powerunits_market_driver_features_bounded_de_validate"
_VALIDATE_PATH = "/internal/hermes/bounded/v1/market-driver-features-hourly/validate-window"
...
def validate_powerunits_market_driver_features_bounded_de_window(
    *,
    ...
    base_statement = (
        "**DE** bounded **`market_driver_features_hourly`** validation. Read-only Repo B POST; "
        "**`country_code=DE`**; **≤24h** window (exclusive end); separate from **`market_features`** tools."
    )

    if not check_powerunits_market_driver_features_bounded_de_validate_requirements():
        return json.dumps(
            {
    ...
    try:
        ...
        logger.warning("market_driver_features DE bounded validate: HTTP timeout")
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
        ...
        logger.warning("market_driver_features DE bounded validate: HTTP error %s", e)
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
...
VALIDATE_MDRIVER_DE_SCHEMA = {
    "name": "validate_powerunits_market_driver_features_bounded_de_window",
    "description": (
        "**Bounded DE `market_driver_features_hourly` validate-window** — **DE / v1 / ≤24h**, then "
        "one read-only Repo B POST. Separate from **`HERMES_POWERUNITS_MARKET_FEATURES_BOUNDED_DE_*`** / Option D. "
...
registry.register(
    name="validate_powerunits_market_driver_features_bounded_de_window",
    toolset="powerunits_market_driver_features_bounded_de_validate",
    schema=VALIDATE_MDRIVER_DE_SCHEMA,
    handler=lambda args, **kw: validate_powerunits_market_driver_features_bounded_de_window(
    ...
    check_fn=check_powerunits_market_driver_features_bounded_de_validate_requirements,
    requires_env=[
        MARKET_DRIVER_FEATURES_BOUNDED_PRIMARY_ENV,
        MARKET_DRIVER_FEATURES_BOUNDED_ALLOWED_COUNTRIES_ENV,
        _LEGACY_ENV,
