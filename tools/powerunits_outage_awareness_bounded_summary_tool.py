#!/usr/bin/env python3
"""Hermes bounded **outage awareness** summary-window — one read-only POST to Repo B."""
...
from tools.powerunits_bounded_family_gates import (
    ...
    outage_awareness_bounded_core_step_enabled,
...
_STEP = "summary"
_LEGACY_ENV = OUTAGE_AWARENESS_BOUNDED_LEGACY_ENV[_STEP]
...
_SURFACE = "powerunits_outage_awareness_bounded_summary"
_SUMMARY_PATH = "/internal/hermes/bounded/v1/outage-awareness/summary-window"
_DEFAULT_TIMEOUT_S = 120
_MAX_SUMMARY_CHARS = 8500
...
def check_powerunits_outage_awareness_bounded_summary_requirements() -> bool:
...
def summarize_powerunits_outage_awareness_bounded_window(
    *,
    country: str,
    start: str,
    end: str,
    ...
    if not check_powerunits_outage_awareness_bounded_summary_requirements():
        return json.dumps(
            {
                "error_code": "feature_disabled",
                "surface": _SURFACE,
                "message": (
                    f"{outage_awareness_bounded_gate_requirement_text(_STEP)}; "
    ...
    base_statement = (
        "**Read-only outage awareness summary.** Hermes performs no SQL. Exactly **one** HTTP POST to Repo B "
        "`…/outage-awareness/summary-window`. **Does not start** outage ingestion (**Step A**), "
        "**does not run** **`outage_country_hourly_compute`**, **`market_feature_job`**, "
    ...
    try:
        cc, start_dt, end_dt = validate_outage_awareness_bounded_slice(
    ...
    try:
        ...
        logger.warning("outage awareness bounded summary: HTTP timeout")
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
        ...
        logger.warning("outage awareness bounded summary: HTTP error %s", e)
        return json.dumps(
            {
                "surface": _SURFACE,
                "slice": slice_obj,
    ...
    summary_text = _redact_secrets(resp.text or "")
    ...
    out: dict[str, Any] = {
        ...
        "response_body_summary": summary_text,
...
SUMMARY_OUTAGE_AWARENESS_SCHEMA = {
    "name": "summarize_powerunits_outage_awareness_bounded_window",
    "description": (
        "**Bounded DE outage awareness summary-window** — read-only, DE / v1 / ≤7d; one HTTP POST. "
        "Composes Repo B validation + classifier; **no** jobs. "
        f"Gate `{OUTAGE_AWARENESS_BOUNDED_PRIMARY_ENV}` or `{_LEGACY_ENV}`; optional "
        f"`{OUTAGE_AWARENESS_BOUNDED_ALLOWED_COUNTRIES_ENV}`; {_BASE_ENV}, {_SECRET_ENV}."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "pipeline_run_id": {"type": "string", "description": "Optional pipeline run UUID echo."},
        },
...
registry.register(
    name="summarize_powerunits_outage_awareness_bounded_window",
    toolset="powerunits_outage_awareness_bounded_summary",
    schema=SUMMARY_OUTAGE_AWARENESS_SCHEMA,
    handler=lambda args, **kw: summarize_powerunits_outage_awareness_bounded_window(
    ...
    check_fn=check_powerunits_outage_awareness_bounded_summary_requirements,
    requires_env=[
        OUTAGE_AWARENESS_BOUNDED_PRIMARY_ENV,
        OUTAGE_AWARENESS_BOUNDED_ALLOWED_COUNTRIES_ENV,
        _LEGACY_ENV,
