...
- [ ] **Preflight — primary:** `HERMES_POWERUNITS_ENTSOE_FORECAST_BOUNDED_ENABLED=1` → `preflight_powerunits_entsoe_forecast_bounded_slice` (Tier **`DE`/`NL`/`BE`/`FR`/`AT`** / **`v1`** / ≤7 d) → `syntactically_valid: true`, bounded hint names execute tool.
...
- [ ] **Gate on:** **`inventory_powerunits_bounded_coverage_v1`** with **`[window_start_utc, window_end_utc)`**, ≤31 d UTC span, **`country_codes`** (one or several **Tier‑1 bounded ERA5** ISO2, e.g. `DE`, `NL`, `IT` — Repo B denies unknown ERA5 bbox keys with explicit skipped rows); omit **`families`** → Repo B returns **four** default inventory families (**ERA5**, ENTSO‑E **market**, **outage awareness**, ENTSO‑E **forecast**). Expect **`skipped`** rows for ISO2 outside each family Repo B v1 allowlist (notably **`bounded_outage_awareness_v1`** remains **DE-only**; bounded **ENTSO‑E market / forecast Tier v1** is **`DE`/`NL`/`BE`/`FR`/`AT`** — e.g. **ES** / **IT** stay **`skipped`** until allowlisted separately); **`repo_b_inventory`** JSON stays canonical (**no Hermes matrix**).
