...
| **Repo B file read** (`read_powerunits_repo_b_allowlisted`) | Allowed **only** when gated | `HERMES_POWERUNITS_REPO_B_READ_ENABLED` + GitHub read token; keys only from `config/powerunits_repo_b_read_allowlist.json`; GitHub API only, no free paths. |
