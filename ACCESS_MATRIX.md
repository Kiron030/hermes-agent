...
| **Country coverage inspect** (`inspect_powerunits_country_coverage_v1`) | Allowed **only** when gated | **`HERMES_POWERUNITS_COUNTRY_COVERAGE_INSPECT_ENABLED`** + same execute base URL and bearer (**no** **`DATABASE_URL_TIMESCALE`** on this tool); **one** read-only **`POST …/country-coverage/inspect`**. Generic ISO2 + bounded dataset catalog. **No** SQL from Hermes, **no** jobs, **no** writes. |
| **DB health observe** (`read_powerunits_db_health_*_v1`, `read_powerunits_timescale_observe_v1`) | Allowed **only** when gated | **`HERMES_POWERUNITS_DB_HEALTH_READ_ENABLED`** + same execute base URL and bearer (**no** **`DATABASE_URL_TIMESCALE`** on these tools); **named** read-only **`POST …/db-health/{storage,planner,indexes,vacuum,sessions,statements,timescale}`**. Catalog/stat views only. **No** SQL from Hermes, **no** jobs, **no** writes, **no** advisor. |
