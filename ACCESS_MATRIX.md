...
| **Option D bounded execute** (`execute_powerunits_option_d_bounded_slice`) | Allowed **only** when gated | `HERMES_POWERUNITS_OPTION_D_EXECUTE_ENABLED`; **one** subprocess: `python -m tools.powerunits_option_d_bounded_market_features` with validated args only — **no** direct SQL from Hermes, **no** other jobs/shells from this tool; not a general writer. |
