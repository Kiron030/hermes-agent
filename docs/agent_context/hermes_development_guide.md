> **Relocated from root `AGENTS.md` (Agent Context Efficiency Audit v1).**
> Always-on contract: root `AGENTS.md`. This file is the full on-demand development guide.
...
- **Repo A (`hermes-agent`):** Hermes **runtime** — gateway, tools, agent loop, Railway-oriented internal images and Powerunits-first-safe policy. This is where integration and bounded operator surfaces live.
- **Repo B (Powerunits product repo, e.g. EU-PP-Database):** **Product, data platform, APIs, migrations** — canonical for schemas, pipelines, and production DB behavior. Do not treat Repo A as the source of truth for those.
...
**Operator / stage docs (Powerunits):** `SOUL.hermes.md` (Stage 1 Trusted Analyst profile), `RUNBOOK.hermes-trusted-analyst.md`, `ACCESS_MATRIX.md`, `RUNBOOK.hermes-stage1-validation.md` (post-deploy checks), `docs/powerunits_hermes_progressive_posture_v1.md` (**canonical staged roadmap**, Phase 0 + Phase 1A/1B/2A/2B + **Tier 3** skills observer + **Tier 4A** draft proposals), `docs/powerunits_operator_posture_diagnostics_v1.md` (Phase 1B posture tool + overlay signals through **Tier 4A**), `docs/powerunits_phase2a_tier1_workspace_analysis_overlay_v1.md` (Phase **2A** Tier‑1 read overlay), `docs/powerunits_phase2b_tier2_allowlisted_locals_overlay_v1.md` (Phase **2B** Tier‑2 allowlisted-local read overlay), `docs/powerunits_tier3_skills_integration_overlay_v1.md` (**Tier 3** capability env `3` skills integration), `docs/powerunits_tier4a_skill_draft_proposals_overlay_v1.md` (**Tier 4A** capability env `4` skill draft proposals), `docs/powerunits_timescale_read_operator_v1.md`, `docs/powerunits_repo_b_read_operator_v1.md` (Repo B read prep — **no live tool yet**), `config/powerunits_repo_b_read_allowlist.json`. **Stage 2 (documentation only, not live):** `SOUL.hermes-writer.md`, `RUNBOOK.hermes-writer.md`, `CHECKLIST.hermes-writer-activation.md` (mandatory gate before any future enable).
