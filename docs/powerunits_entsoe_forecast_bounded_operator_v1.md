...
**Repo B bounded v1:** Tier‑1 ISO2 **`DE`** or **`NL`**, **`version=v1`** — matches Repo B `entsoe_forecast_job` slice contract (**Hermes mirrors** the same allowlist for local preflight + **`HERMES_POWERUNITS_ENTSOE_FORECAST_BOUNDED_ALLOWED_COUNTRIES`** narrowing when primary is on).
...
**After execute:** Repo B runs **`entsoe_forecast_job.run`** only (bounded slice **`DE`/`NL`**, **v1**, `[window_start_utc, window_end_utc)` exclusive end, **≤ 7 days** UTC).
...
Optional **`HERMES_POWERUNITS_ENTSOE_FORECAST_BOUNDED_ALLOWED_COUNTRIES`** (comma-separated ISO2). **Unset** ⇒ Hermes **implicit `DE` only** per-request intersection; **empty string** ⇒ **fail-closed** when primary is truthy. **`DE,NL`** to allow both Tier‑1 forecast countries with primary gating.
