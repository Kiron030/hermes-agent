...
**`country_code=DE`** and **`version=v1`** only — matches Repo B bounded `entsoe_forecast_job` slice contract.
...
**After execute:** Repo B runs **`entsoe_forecast_job.run`** only (bounded slice **DE**, **v1**, `[window_start_utc, window_end_utc)` exclusive end, **≤ 7 days** UTC).
...
Optional **`HERMES_POWERUNITS_ENTSOE_FORECAST_BOUNDED_ALLOWED_COUNTRIES`** (comma-separated ISO2). **Unset** ⇒ implicit **DE** for current tools; **empty string** ⇒ **fail-closed** when primary is truthy.
