...
| **ENTSO‑E BZN readiness read** | **`HERMES_POWERUNITS_ENTSOE_BZN_PRICE_READINESS_READ_ENABLED`** | **Read-only** Repo B **`POST …/entsoe-bzn-price-readiness/read`** (advisory ISO2 batch); same **`POWERUNITS_INTERNAL_EXECUTE_*`** credentials as other bounded internals; Hermes performs **no** jobs or writes. |
...
| **DB health observe** | **`HERMES_POWERUNITS_DB_HEALTH_READ_ENABLED`** | **Read-only** PostgreSQL/Timescale catalog observe — Repo B **`…/db-health/*`**. Same bounded execute **base URL** + bearer. **No** SQL from Hermes, **no** Timescale DSN on these tools. Included in **`stage1_read_health`**. |
...
Same rules as §4 (**primary wins**, **legacy when primary falsy**, **allowlist ignored on legacy-only**) for steps **preflight**, **execute**, **validate**, **summary**. **ENTSO‑E market + forecast (primary):** **`HERMES_*_ALLOWED_COUNTRIES` absent** ⇒ Hermes permits **all** ISO2 in the Repo B Tier‑v1 mirror (**`{DE,NL,BE,FR,AT,CZ,PL,FI}`** today); **non‑empty** ⇒ intersection with that mirror (intentional narrowing); **explicit empty** ⇒ fail‑closed when primary is on. ISO2 **outside** Repo B Tier‑v1 is always rejected. **ENTSO‑E forecast bounded** uses the same permit model (`…_FORECAST_BOUNDED_ALLOWED_COUNTRIES`).
...
Core steps align with **bounded ENTSO‑E market** (**primary** + optional allowlist; **absent allowlist ⇒ Tier‑1 mirror**; **explicit empty ⇒ fail-closed**); optional allowlist **intersects** Repo B Tier v1 **`DE`/`NL`/`BE`/`FR`/`AT`**. **Orthogonal** to **`ENTSOE_MARKET_BOUNDED_*`** — routes `…/entsoe-forecast/*` invoke **`entsoe_forecast_job`** only (no **`entsoe_market_job`**, no market-features auto-run).

Recommended primary:

- **`HERMES_POWERUNITS_ENTSOE_FORECAST_BOUNDED_ENABLED=1`**
- Optional **`HERMES_POWERUNITS_ENTSOE_FORECAST_BOUNDED_ALLOWED_COUNTRIES`** (comma ISO2 subset of Repo B **`DE`/`NL`/`BE`/`FR`/`AT`**; **omit** ⇒ full Tier‑1 mirror at Hermes; empty ⇒ fail-closed with primary).
...
### DB health observe v1 (read-only catalog/stat views)

Hermes **`read_powerunits_db_health_*_v1`** / **`read_powerunits_timescale_observe_v1`** → Repo B **`POST /internal/hermes/bounded/v1/db-health/{storage,planner,indexes,vacuum,sessions,statements,timescale}`**. **Gate:** **`HERMES_POWERUNITS_DB_HEALTH_READ_ENABLED`**. Telegram toolset **`powerunits_db_observe`** is in **`first_safe_v1`** base overlays and **`stage1_read_health`**. Allowlisted relations + bounded top-N; **no** SQL from Hermes. See **`docs/powerunits_operator_observability_2_db_health_v1.md`**.
