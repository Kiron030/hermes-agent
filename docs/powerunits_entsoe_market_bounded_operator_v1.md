...
**Bounded v1 ISO2 set (Repo B authoritative):** **`DE`**, **`NL`**, **`BE`**, **`FR`**, **`AT`**, **`CZ`**, **`PL`**, **`FI`**. Hermes mirrors this set in code; optional
...
European expansion sequencing (paired with forecast; **planning docs only**) lives in Repo B **`docs/adr/045_bounded_entsoe_rollout_scaling_model_at_lessons_and_candidate_matrix.md`** and **`services.internal.hermes_bounded_entsoe_candidate_readiness_matrix_v1`**.
...
- **Repo B Tier v1 **`DE`/`NL`/`BE`/`FR`/`AT`/`CZ`/`PL`/`FI`**, **v1**. Range `[scan_start_utc, scan_end_utc)` with **exclusive** end — same **≤ 31 d** span and **≤ 5** contiguous **≤ 7 d** sub-windows as the bounded campaign partitioning.
...
- **Repo B Tier v1 **`DE`/`NL`/`BE`/`FR`/`AT`/`CZ`/`PL`/`FI`**, **v1** only (per-slice `country_code`). Campaign range `[campaign_start_utc, campaign_end_utc)` with **exclusive end**; **total span ≤ 31 days**; split into **contiguous** sub-windows each **≤ 7 days** (**≤ 5** sub-windows).
