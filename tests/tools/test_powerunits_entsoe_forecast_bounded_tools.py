...
def test_execute_forecast_blocked_when_allowlist_explicit_de_through_at_blocks_cz_pl_fi(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_fcst_bounded_core(monkeypatch)
    monkeypatch.setenv(ENTSOE_FORECAST_BOUNDED_PRIMARY_ENV, "1")
...
def test_execute_forecast_blocked_when_allowlist_pre_hu_sk_ro_excludes_hu_sk_ro(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_fcst_bounded_core(monkeypatch)
    monkeypatch.setenv(ENTSOE_FORECAST_BOUNDED_PRIMARY_ENV, "1")
