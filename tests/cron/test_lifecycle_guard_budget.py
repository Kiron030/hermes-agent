...
def test_check_gateway_lifecycle_python_path_charges_masker(monkeypatch, tmp_path):
    """The .py branch's direct scan tokenizes too, so it is budgeted and fails
    closed before shlex on an over-budget line. (Upstream charges its
    data-exemption masker here; the fork does not port the masker, the budget
    charge stays.)"""
