...
def test_model_tools_inventory_visible_when_gate_on(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("HERMES_POWERUNITS_RUNTIME_POLICY", "first_safe_v1")
    ...
    enabled = _get_platform_tools({}, "telegram")
    assert "powerunits_bounded_coverage_inventory" in enabled
    names = {
        d["function"]["name"]
        for d in get_tool_definitions(enabled_toolsets=sorted(enabled), quiet_mode=True)
    }
    assert "inventory_powerunits_bounded_coverage_v1" in names
