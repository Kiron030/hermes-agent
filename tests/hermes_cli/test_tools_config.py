...
def test_telegram_first_safe_merges_bzn_prices_toolset(monkeypatch):
    monkeypatch.setenv("HERMES_POWERUNITS_RUNTIME_POLICY", "first_safe_v1")
    enabled = _get_platform_tools({}, "telegram")
    assert "powerunits_entsoe_bzn_prices" in enabled


def test_telegram_first_safe_bzn_prices_respects_agent_disabled_toolsets(monkeypatch):
    monkeypatch.setenv("HERMES_POWERUNITS_RUNTIME_POLICY", "first_safe_v1")
    config = {"agent": {"disabled_toolsets": ["powerunits_entsoe_bzn_prices"]}}
    enabled = _get_platform_tools(config, "telegram")
    assert "powerunits_entsoe_bzn_prices" not in enabled


def test_telegram_first_safe_resolves_bzn_prices_tool_in_get_tool_definitions(monkeypatch):
    monkeypatch.setenv("HERMES_POWERUNITS_RUNTIME_POLICY", "first_safe_v1")
...
def test_telegram_first_safe_bzn_prices_not_in_schema_when_gate_off(monkeypatch):
    monkeypatch.setenv("HERMES_POWERUNITS_RUNTIME_POLICY", "first_safe_v1")
