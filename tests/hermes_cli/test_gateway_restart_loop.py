...
Guard tests are ported from release tag v2026.9.7 (MUSTPORT-5B). Cases the tag expects to be
ALLOWED but the fork keeps blocking (see cron/lifecycle_guard.py module docstring) are asserted as
blocked here and marked "fork". fork-local regressions live in
tests/cron/test_lifecycle_guard_fork_regressions.py.
"""
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        # fork: the tag allows these (#92372 Branch D leading \b; #77173/#77536
        # `/` in the Branch A lookbehind). The same relaxations also let
        # `skill`/`fkill` and a path-invoked `/usr/local/bin/hermes gateway
        # restart` through, so the pre-port blocking verdict is kept.
    ...
    def test_heredoc_body_prose_still_blocked(self):
        # fork: the tag masks quoted-delimiter heredoc bodies fed to
        # cat/python/osascript (#88336). python/osascript execute the body and
        # a written file can be executed later, so the body is still scanned.
        text = (
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    Ported from the tag with the fork's gate (the `_HERMES_GATEWAY` marker, not
    the tag's supervised-process probe) and the fork's guard cwd (the command's
    execution cwd, so relative references are pinned via `workdir`).
    """
    ...
    def test_relative_script_uses_command_workdir(self, monkeypatch, tmp_path):
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(
            command="/bin/bash relative.sh", workdir=str(tmp_path)
        ))
    ...
    def test_shell_c_payload_recursively_scans_script(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command="/bin/bash -c '/bin/bash nested.sh'", workdir=str(tmp_path)
        ))
...
class TestDataSinkArgumentsKeepBlocking:
    """fork: the tag's data-sink exemption (lifecycle text inside grep/rg/
    sqlite3/psql/journalctl/ag/ack arguments) is not ported, so these keep the
    pre-port blocking verdict. The tag's execution-route cases still block."""
    ...
    def test_lifecycle_text_in_data_arguments_still_blocks(self, command):
        assert self._scan(command) is True

    @pytest.mark.parametrize("command", [
        # No command shape at all: allowed before the port and after it.
    ...
    def test_sql_without_a_command_shape_is_allowed(self, command):
    ...
    def test_execution_routes_still_block(self, command):
        assert self._scan(command) is True

    def test_python_script_branch_still_scans_prompt_sql(self, tmp_path):
        """fork inverse of the tag's exemption test: the .py branch scans the
        combined prompt+script text, and lifecycle text in the prompt blocks."""
        from cron.lifecycle_guard import GatewayLifecycleBlocked, check_gateway_lifecycle
