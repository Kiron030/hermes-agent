...
def test_apply_policy_excludes_phase2a_toolset_when_capability_tier_zero(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "powerunits_tier5a_bounded_workflow_scaffolding" not in data["platform_toolsets"][
            "telegram"
        ]
...
def test_apply_policy_telegram_always_includes_entsoe_bzn_readiness_and_prices_leaf_toolsets() -> (
    ...
    assert "powerunits_country_coverage_inspect" in tg
...
def test_apply_policy_includes_phase2b_toolset_when_capability_tier_ge_two(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "powerunits_tier4a_skill_draft_proposals" not in tg
...
def test_apply_policy_includes_tier4a_when_capability_tier_is_four(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    yaml = pytest.importorskip("yaml")
    apply_policy = _load_apply_policy()
...
def test_apply_policy_includes_tier4b_when_capability_tier_is_five(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "powerunits_tier5a_bounded_workflow_scaffolding" not in tg


def test_apply_policy_includes_tier5a_when_capability_tier_is_six(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    yaml = pytest.importorskip("yaml")
    apply_policy = _load_apply_policy()
