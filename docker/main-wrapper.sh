...
# Phase 0 Progressive Liberation: optional capability tier label for logs
# only (tier>0 does not change runtime until explicitly wired). See
# docs/powerunits_hermes_progressive_posture_v1.md
s6-setuidgid hermes /opt/hermes/.venv/bin/python3 -c \
    "from powerunits_capability_tier import log_startup_capability_tier_notice; log_startup_capability_tier_notice()" \
    || true
