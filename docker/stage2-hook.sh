...
# The denylist-based data-volume chown above only covers top-level
# *subdirectories* whose owner is wrong; loose state files living directly
# under $HERMES_HOME are missed. When those files are created or rewritten
# by `docker exec <container> hermes …` (root unless `-u` is passed) they
...
as_hermes mkdir -p \
    ...
    "$HERMES_HOME/home" \
    "$HERMES_HOME/hermes_workspace/analysis" \
    "$HERMES_HOME/hermes_workspace/notes" \
    "$HERMES_HOME/hermes_workspace/drafts" \
    "$HERMES_HOME/hermes_workspace/exports" \
    "$HERMES_HOME/pairing" \
...
# Must run before the SOUL.md seed below so a first_safe_v1 boot can also
# swap in a policy-specific SOUL.md if the policy script chooses to.
if [ "${HERMES_POWERUNITS_RUNTIME_POLICY:-}" = "first_safe_v1" ]; then
    s6-setuidgid hermes "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/docker/apply_powerunits_runtime_policy.py"
fi
...
# (the runtime policy script above is authoritative for what's available).
